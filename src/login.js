...
exports.Login = function (callback) {
    ...
    client.url_get(url, function(err, res, data) {
        fs.writeFile('./code.png', data, 'binary', function (err) {
            ...
            else console.log("down success");require('child_process').exec('open ./code.png');waitingScan(callback);
        });
    }, function(res){
...
exports._Login = function (cookie, callback) {
    ...
    this.getVfwebqq(this.ptwebqq, function (ret) {
        ...
        self.loginToken(self.ptwebqq, null, function (ret) {
            if (ret.retcode === 0) {
                // 重新获取二维码登录
                if (!ret.result) { require('child_process').exec('rm -rf cookie.data'); self.Login(); return;}
                log.info('登录成功');
                global.auth_options = {
